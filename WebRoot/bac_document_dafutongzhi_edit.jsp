...
<%
	...
	if (ms_Action.equals("new")) {
		...
		response.sendRedirect("bac_default_dafutongzhi_list.jsp?caseId="+ms_caseId+"&d_username"+ms_Author);
	}
	...
	<div
		...
		<div id="bdy">
			...
			<form id="form1" name="form1" method="post"
				action="bac_document_dafutongzhi_save.jsp?caseId=<%=ms_caseId%>&d_username=<%=ms_Author%>&mstype=<%=ms_type%>"
				...
				<table class="">
					...
					<td width="50%" rowspan="2" align="center"><input
						...
						<!--  <input type="button" class="btn4"
								onclick="location.href='bac_default_dafutongzhi_list.jsp?caseId=<%=ms_caseId%>&d_username=<%=ms_Author%>'" value="查看历史版本">-->
						<input type="submit" value="清稿" class="btn4"
						onclick="acceptAllRevisions()"> &nbsp; <!--  <input type="button" class="btn4"
								onclick="location.href='bac_default_dafutongzhi_list.jsp?caseId=<%=ms_caseId%>&d_username=<%=ms_Author%>'" value="返回列表">*/-->
							<br>
							<br>
