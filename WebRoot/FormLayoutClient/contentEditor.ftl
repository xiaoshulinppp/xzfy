...
<title>表单布局工具</title>
<link href="../${Session["
	_USER_STYLE_"].cssPath}/style.css" rel="stylesheet" type="text/css">
</head>
<Script Language="Javascript" src="js/colorEditor.js"></Script>
<body>
	<form name="'designForm'">
		<table width="100%" border="0" align="center" cellpadding="0"
			cellspacing="0" class="yourPlace">
			<tr>
				<td height="20"><table width="100%" border="0" cellspacing="0"
						cellpadding="0">
						<tr>
							<td width="30" align="center"><img src="../${Session["
								_USER_STYLE_"].imagePath}/icon_yourplace.gif" width="14"
								height="12"></td>
							<td>您的位置:表单单元编辑器</td>
						</tr>
					</table></td>
		...
		<table width="100%" height="90%">
			<tr height=10%>
				<td><select language="javascript"
					onchange="document.execCommand('fontsize',true,this[this.selectedIndex].value);">
						<option>字号
						<option value="1">1
						<option value="2">2
						<option value="3">3
				...
				<td>
					<!--��ɫ��-->
					<div id=colorTableDiv
						style="LEFT: 200 px; POSITION: absolute; TOP: 40px; display: none; width: 200; Height: 100; background-color: #ddeeff">
						<table>
							<tr>
						...
						<table id=colorTable onmousedown="colorEditor.setColor();"
							onmousemove="colorEditor.setSelected()" cellspacing="1px"
							cellpadding="0px" bgColor="#000000" style="cursor: hand;"></table>
					</div>
				</td>
			</tr>
			<tr height=80%>
				<td colspan=14>
					<div id="cellDesignArea" style="display: block; height: 100%"
						contenteditable></div>
		...
		<input id=button_ok type="button" class="button"
			onclick="webDesignEnd();" value="确定" /> <input id=button_cancel
			type="button" class="button" onclick="window.close();" value="取消" />
		<input id=button_design type="button" class="button"
			onclick="viewDesign();" disabled value="编辑" /> <input id=button_code
			type="button" class="button" onclick="viewCode();" value="代码" />
</body>
</html>
<script>
function viewDesign(){
