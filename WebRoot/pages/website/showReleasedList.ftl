		    ...
		    <table id=""class="ui-table ui-table-search  "  summary="这是一个1行4列的表格样式模板">
		        ...
		        <tr>
		         ...
		         <td ><input class="ui-input-wdm" type="text"  name="createdDate" value="${createdDate?default('')}" onClick="WdatePicker()"/></td>
		          <td >发布时间</td>
		          <td ><input class="ui-input-wdm" type="text" name="releaseDate" value="${releaseDate?default('')}"  onClick="WdatePicker()" /></td>
...
<#elseif info.column=="4">典型案例<#elseif info.column=="5">统计查询<#elseif info.column=="6">工作管理<#elseif info.column=="7">在线调查<#else>软件资料</#if></td>
<td>&nbsp;${info.viewCount?default(0)}</td>
<td>&nbsp;${info.resource?default('')}</td>
<td>&nbsp;<#if isTop=="0"><a href="../website/setTop.action?funcCode=${funcCode}&articleId=${info.id}&column=${column}" title="点击使置顶">未置顶</a>
<#else><a href="../website/unSetTop.action?funcCode=${funcCode}&articleId=${info.id}&column=${column}" title="取消置顶">已置顶</a></#if></td>
<td><a href="../website/showArticle.action?funcCode=${funcCode}&articleId=${info.id}" target="_blank">查看</a></td>
<td>&nbsp;<a href="../website/deleteArticle1.action?funcCode=${funcCode}&articleId=${info.id}&column=${column}">删除</a></td>
</tr>
</#list>
</tbody>
</table>
...
<script language="javascript">
			...
			function search(pNum){
				...
				document.form.action="showReleasedList.action"
