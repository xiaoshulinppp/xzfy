...
<body onload="ChangeFileType()">
	<div id="bdy">
		...
		<div id="nav">
			您当前位置 &gt;&gt; <a
				href="bac_default_cailiaochulidan.jsp?caseId=<%=ms_cassid%>&d_username=<%=ms_userrName%>&d_checkstatus=<%=ms_checkstatus%>">首页</a>
		</div>
		<hr />

		<table class="lst">
			<tr>
				<th colspan="2">材料处理单编辑管理——新版笔录编辑</th>
			</tr>
			<tr>
				<td>
					<form name="form1"
						action="bac_document_cailiaochulidan_edit.jsp?action=new"
						method="post">
						用户名： <input name="d_username" id="d_username" style="width: 75px"
							type="text" value="<%=ms_userrName%>" /> <input type=button
							value="编辑材料处理单" onclick="printNotice()">
						<script>
<%!//首先用模板管理新建模板，并添加相关书签，然后修改相关参数，包括d_username、d_templateid、caseId 
	public String tempUrl;
	//秘书名称
	public String d_templateid = "20131128154211";//模板编号
	//案件ID%>
<%tempUrl = request.getContextPath();
			tempUrl = request.getScheme() + "://" + request.getServerName()
					+ ":" + request.getServerPort() + tempUrl + "/";
			tempUrl += "bac_document_cailiaochulidan_edit.jsp?action=new&d_filetype=doc&d_templateid="
					+ d_templateid
					+ "&d_username="
					+ ms_userrName
					+ "&caseId="
...
function printNotice(){	
	window.open("<%=tempUrl%>");
		...
		<table class="lst">
			...
			<%
					String sql = "select * from (select * from eWebOffice_T_Document where D_TYPE='材料处理单'  and caseId="+ms_cassid+"and D_SHENPIDANTYPE="+ms_checkstatus+" order by d_id desc ) where rownum=1 ";
					try {
						con = getConn();
						...
						if (length == 0)
						{
							s_alert = "<script>window.location='bac_document_cailiaochulidan_edit.jsp?action=new&d_filetype=doc&d_templateid="+d_templateid+"&caseId="+ms_cassid+"&d_username="+ms_userrName+"&d_checkstatus="+ms_checkstatus+"'</script>";
							response.getWriter().write(s_alert);
						}
						while (rs.next()) {
							if(rs.getString("D_isqinggao").equals("未清稿")){
								s_alert = "<script>window.location='bac_document_cailiaochulidan_edit.jsp?action=yesrevision&caseId="+ms_cassid+"&d_username="+ms_userrName+"&d_recordid="+rs.getString("D_RecordID")+"'</script>";
							response.getWriter().write(s_alert);
							}
							if(rs.getString("D_isqinggao").equals("已清稿")){
								s_alert = "<script>window.location='bac_document_cailiaochulidan_edit.jsp?action=read&caseId="+ms_cassid+"&d_username="+ms_userrName+"&d_recordid="+rs.getString("D_RecordID")+"'</script>";
							response.getWriter().write(s_alert);
							}
							String s_Link = "";
							if (rs.getString("D_FileType").toLowerCase().equals("doc")
									|| rs.getString("D_FileType").toLowerCase().equals(
											"wps")) {
								s_Link = "<a href='bac_document_cailiaochulidan_edit.jsp?action=norevision&d_templateid="
										+ d_templateid
										+ "&d_username="
										+ ms_userrName
										+ "&d_recordid="
										...
										+ ms_cassid
										+ "'>修改无痕迹</a>"
										+ "→<a href='bac_document_cailiaochulidan_edit.jsp?action=yesrevision&d_templateid="
										+ d_templateid
										+ "&d_username="
										+ ms_userrName
										+ "&d_recordid="
										...
										+ ms_cassid
										+ "'>修改有痕迹</a>"
										+ "→<a href='bac_document_cailiaochulidan_edit.jsp?action=read&d_templateid="
										+ d_templateid
										+ "&d_username="
										+ ms_userrName
										+ "&d_recordid="
								...
								s_Link = "<a href='bac_document_cailiaochulidan_edit.jsp?action=norevision&d_username="
										+ ms_userrName
										+ "&d_recordid="
										+ rs.getString("D_RecordID")
										+ "&caseId="
										...
										+ "→套红"
										+ "→签章"
										+ "→<a href='bac_document_cailiaochulidan_edit.jsp?action=read&d_username="
										+ ms_userrName
										+ "&d_recordid="
										+ rs.getString("D_RecordID")
										+ "&caseId="
