...
ymPrompt.prototype._createWin=function(title,content,btnType){
	...
	//第一次需要创建一个容器
	//总容器的样式
	outerStyle="position:absolute;top:50%;left:50%;margin-left:"+(-this._width/2+this._rootEl.scrollLeft)+"px;width:"+this._width+"px;margin-top:"+(-this._height/2+this._rootEl.scrollTop)+"px;z-index:10001";
	...
	//alert("第一次");
		...
		//内容容器层
		var content_div="<div style='width:100%;overflow:visible' id='promptContent'><table cellpadding=0 cellspacing=0 border=0 style='margin:0px;padding:0px;background-color:#F4F4F4;' align=center width='100%' height='125' id='promptContentTable'><tr><td id='winMiddleLeft' width='3'>&nbsp;</td><td id='winMiddleCenter' style='background:url("+this._promptSkin+"/images/"+this._icon+") no-repeat 20px 50%'>&nbsp;</td><td id='winMiddleRight' width='3'>&nbsp;</td></tr>";
		if(btnType){	//显示的按钮的类型
		...
		//this._addCSS(outContainer,myStyle);	//添加样式
	...
	if(navigator.userAgent.indexOf("MSIE 7.0")>-1||navigator.userAgent.indexOf("MSIE 8.0")>-1){	//2007-10-10 For Only IE7，IE8
		...
		_h=(_h>parseInt($obj("promptWinContainer").style.height)?(_h+"px"):150);
		$obj("promptWinContainer").style.width=$obj("titleContainer").style.width=$obj("promptContent").style.width=_w;
		$obj("promptWinContainer").style.height=_h;
...
ymPrompt.prototype._showMask=function(){
	...
	if(!$obj("promptShield")){
		...
		//	alert($obj("promptShield").style.height);
	...
	$obj("promptShield").style.display="none";	//2007-11-13 如果显示则先隐藏便于后面计算页面的高宽
